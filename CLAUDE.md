...
- Navigation: **mobile** uses the fixed 4-tab bottom nav (Suche/Start/Üben/Mehr) — Suche opens the command palette, Mehr (`/more`) is a 2-col grid linking to Wörter/Pakete/Zeichen/Einstellungen + a wide About card. **Desktop (`lg:`)** replaces the tab bar with a persistent 240px left sidebar (Start/Üben · Wörter/Pakete/Zeichen · Einstellungen) + a ⌘K search button; the outer wrapper widens from 768px → 1080px. Both paths share the same `isActive()` logic in `(app)/+layout.svelte`.
- Dashboard: per-source drill cards (each pack + lexicon), "Alles gemischt" button, minimal today stats inline
...
- Wörter (`/lexicon`): one input does triple duty — filters the saved lexicon, searches the dictionary (unified ZH/EN/DE), and adds the query as a new word on Enter when it isn't already saved. Dictionary hits render below the lexicon rows with add/remove toggle per result; tapping a saved row expands its Wiktionary entry inline. Absorbed the former standalone `/dictionary` route on 2026-05-25. Reads `?q=` on mount and mirrors the query back into the URL (debounced 300 ms — see the August 2026 note above for why that write must compare trimmed-to-trimmed)
...
- Stats: the standalone `/stats` route (bar charts, accuracy trend, rating distribution, 365-day activity heatmap) was **removed 2026-05-25**. `/home` keeps a "Heute" card with reviewed-today count + accuracy from `hanzi_get_drill_stats`; the streak lives in `StreakBanner`. The Zig stats exports are untouched, so the route is re-addable if the charts are ever missed

...
<div class="space-y-6 py-4">
	...
	{#if enabled}
		...
		<div class="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
			...
			<div class="mt-3 flex items-center gap-2">
				<code class="flex-1 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-600">
					{maskedKey(syncKey)}
				</code>
				<button
					onclick={copyKey}
		...
		<div class="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
			<p class="text-sm font-medium text-slate-700">Auf anderem Gerät einrichten</p>
	...
	{:else}
		<!-- Not connected state -->
		<div class="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
			<p class="text-sm font-medium text-slate-700">Neuen Sync starten</p>
			<p class="mt-1 text-xs text-slate-400">
		...
		<div class="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
			<p class="text-sm font-medium text-slate-700">Vorhandenen Schlüssel eingeben</p>
			<p class="mt-1 text-xs text-slate-400">
			...
			<div class="mt-3 flex gap-2">
				<input
					...
					class="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-700 placeholder:text-slate-300 focus:border-primary focus:outline-none"
				/>
				<button
					onclick={linkKey}

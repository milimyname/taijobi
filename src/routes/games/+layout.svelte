...
<main class="grid h-screen grid-cols-1 items-start overflow-auto p-5 lg:grid-cols-[1fr_1fr_6rem]">
	<slot />
	<aside
		...
		>
			{#if $innerWidthStore > IS_DESKTOP}
				...
				<div class="flex gap-10 sm:flex-col">
					...
					<form action="/logout" method="POST" on:click={() => localStorage.clear()}>
						<button type="submit">
							<LogOut class="size-6" />
						</button>
					</form>
